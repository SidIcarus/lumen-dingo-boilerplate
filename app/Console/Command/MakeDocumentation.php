...
class GenerateDocumentationCommand extends Command
{
    ...
    public function __construct()
    {
        ...
        $this->config = [
            ...
            'headerTemplateContent' => file_get_contents(
                resource_path('documentation/shared/header.template.md')
            ),
        ];
    }
    ...
    private function generateHeader($path, $fileName): void
    {
    ...
    private function generateApiDocConfigJson($pathFile): void
    {
        file_put_contents(
    ...
    private function generateAPIDocsTask(): void
    {
...
//        $this->info('Result: ' . $process->getOutput());
    ...
    private function getJsonConfigurationPath(): string
    {
        $template = str_replace(base_path(), '', $this->config['documentFilePath']);

        return substr($template, 1) . 'config/';
    }
}
